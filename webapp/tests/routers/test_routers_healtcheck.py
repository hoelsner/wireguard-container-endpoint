...
class TestHealtcheckEndpoint:
    ...
    """
    async def test_success(self, unauth_test_client: TestClient):
        """test basic healthcheck
        """
        response = await unauth_test_client.get("/api/healthcheck/")
        assert response.status_code == 200
        assert response.json() == {"message": "ok"}

    async def test_wireguard_failure(self, unauth_test_client: TestClient, monkeypatch):
    ...
    async def test_wg_json_failure(self, unauth_test_client: TestClient, monkeypatch):
